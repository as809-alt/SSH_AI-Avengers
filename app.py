...
def get_connection():
    ...
    if not DB_HOST or not DB_USER or not DB_NAME:
        raise RuntimeError(
            "Database is not configured. Create a .env file next to app.py "
            "with DB_HOST, DB_PORT, DB_USER, DB_PASSWORD, DB_NAME set."
        )
...
def get_voucher_for_partner(partner_name):
    ...
    cur.execute(
        ...
        WHERE active = 1 AND stock > 0 AND LOWER(partner_name) = LOWER(%s)
